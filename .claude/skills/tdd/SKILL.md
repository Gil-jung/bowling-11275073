...
## Human-in-the-Loop 워크플로우 (이 프로젝트 전용)

이 저장소에서는 각 사이클마다 아래 3단계를 거치며, **사용자 승인 없이는 RED에서도 REVIEW에서도 commit하지 않는다.**

### 1. RED
1. **목표를 정확히 설정한다** — 이번 사이클에서 구현할 단 하나의 동작(behavior)을 명확히 정의한다.
2. **PLAN.md 생성 후 검토를 요청한다** — 목표, 작성할 실패 테스트, 구현 방향을 PLAN.md에 문서화하고 사용자에게 검토를 요청한다. 사용자 확인 전에는 테스트 코드를 작성하지 않는다.
3. **사용자 검토 후 commit한다** — 사용자가 PLAN.md와 (작성됐다면) 실패하는 테스트를 승인하면 그 상태로 commit한다.

### 2. GREEN
1. RED에서 합의된 PLAN.md의 목표를 달성하도록 최소한으로 구현한다.
2. 테스트를 실제로 실행하여 통과하는지 직접 확인한다 (위 "GREEN 검증" 절차를 그대로 따른다).

GREEN 단계에서는 사용자 승인 없이 다음 REVIEW 단계로 넘어갈 수 있지만, commit은 REVIEW를 거친 뒤에만 한다.

### 3. REVIEW (사용자)
1. 사용자가 GREEN에서 작성된 코드를 리뷰한다.
2. PLAN.md 범위를 벗어난 사항이 구현되지 않았는지 확인한다.
3. 리팩토링이 필요하면 사용자가 요청하고, 요청받은 리팩토링만 수행한다 (테스트는 계속 그린 상태로 유지).
4. 사용자 검토 후 commit한다.

다음 사이클은 다시 RED(새 PLAN.md)부터 시작한다.
